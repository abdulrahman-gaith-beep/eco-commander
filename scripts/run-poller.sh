#!/usr/bin/env bash
# Thin wrapper to run the usage poller from anywhere.
# Used by:
#   • SwiftBar "Refresh now" action
#   • Manual debugging
...
PYTHON_RUNNER="$(select_python_runner)" || {
  echo "run-poller.sh: no supported Python found (requires 3.10-3.13)" >&2
  exit 1
}

exec "$PYTHON_RUNNER" -m poller.main "$@"
